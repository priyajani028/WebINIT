# Notes on MongoDB with Atlas

Link: https://www.youtube.com/watch?v=fbYExfeFsI0&t=392s

## **Steps**
- Create an account on atlas.
- Once done, create a cluster (free wala) and load SampleData.
- After that "Connect", use all defaults, set password and name-> connect with your own ip address -> select "drivers"
- Go back to vs code, install mongodb into your project
  ```
  npm install mongodb
  ```
- now in the app.js, write the basic code: Here the uri is the one given in the drivers wala in atlas. Replace demo with your name used in the settings while connecting, and replace <password> with your own.
  ```
  //jshint esversion:6
  const { MongoClient} = require('mongodb');
  const uri="mongodb+srv://demo:<password>@cluster0.rhv03gd.mongodb.net/?retryWrites=true&w=majority";
  ...
  ```
- In the terminal, run the following command and check if there isn't any error.
  ```
  node app.js
  ```
- now as per the CRUD operations follow all the operations by using each function on your convenience.
...
You can check the results in atlas by quering it.

![image](https://github.com/priyajani028/WebINIT/assets/87660206/36d18835-0ee3-4b6d-8621-08fc4c2390e9)
